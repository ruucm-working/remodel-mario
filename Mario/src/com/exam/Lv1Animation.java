package com.exam;

import android.*;
import android.graphics.*;

public class Lv1Animation implements IAnimatable {
        private Sprite sp = MediaAssets.getInstance().getSprite(R.drawable.samsung_sample);
        private int upstep, movestep;
        private float density;
        
        public Lv1Animation(float Density)
        {
                upstep = 10;
                movestep = 5;
                density = Density;
        }

        public boolean AnimationFinished()
        {
                return movestep<=10;
        }
        
        public void Draw(Bitmap canvas)
        {
        		// 버섯 x,y좌표(right, top)
                int top = -(int)((16-upstep) * 2 * density);
                int right = (int)((10-movestep) * 2  * density);
                SpriteHelper.DrawSprite(canvas, sp, 0, SpriteHelper.DrawPosition.BottomCenter, 0, 0);
                
                boolean direction = true;
                // Two stage
                if (upstep < 10){		// 버섯 올라와
                // Draw sprite
                        upstep++;
                }
                else{		// 다 올라왔으면 오른쪽으로
                	if(movestep == 5)
                        direction = true;
                	else if(movestep == 15)
                		direction = false;
                	
                	if(direction == true)
                		movestep ++;
                	else
                		movestep --;
                }
        }
}